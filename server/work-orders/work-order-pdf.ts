...
const WORK_ORDER_RENDERED_TEXT_EN: Record<string, string> = {
  ...
  "이 항목은 8개의 독립 작업으로 나누기보다 SSR/SSG/사전 렌더링 도입 또는 랜딩 페이지 정적화를 중심 작업으로 묶어 처리해 주세요.": "Treat this as one bundled SSR/SSG, prerendering, or landing-page static rendering task rather than eight separate tasks.",
  "요금제 또는 무료·유료 이용 범위, 개인정보·입력자료 처리 방식, 운영 주체와 문의 경로를 초기 HTML에 1~2문장 이상 요약하고 관련 정책 페이지로 표준 a 링크를 제공해 주세요.": "Summarize pricing or free/paid scope, privacy/input-data handling, operator information, and contact paths in at least 1-2 sentences in the initial HTML, and provide standard anchor links to related policy pages.",
  "실제 화면에 보이는 핵심 FAQ 3~4개를 만들고, 같은 질문·답변을 FAQPage JSON-LD로도 선언해 주세요.": "Create 3-4 key FAQs visible on the actual page, and declare the same questions and answers as FAQPage JSON-LD.",
  "주요 내부 링크는 JavaScript 클릭 핸들러만 쓰지 말고 href가 있는 표준 a 링크로 초기 HTML에 제공해 주세요.": "Provide key internal links in the initial HTML as standard anchor links with href attributes, not only JavaScript click handlers.",
  "초기 HTML 본문이 렌더링 DOM 본문의 75% 이상을 포함하도록 핵심 설명을 앞단에 제공해 주세요.": "Place core explanations upfront so the initial HTML contains at least 75% of the rendered DOM body text.",
  "초기 HTML 본문은 최소 200자 이상이어야 하며, 별도 답변 기반 규칙은 800자를 내부 참고 기준으로 사용합니다.": "The initial HTML body should contain at least 200 characters, and the separate answerability rule uses 800 characters as an internal reference target.",
  "주요 내부 링크는 렌더링 DOM 링크의 75% 이상을 포함하거나 차이가 2개 이하가 되도록 제공해 주세요.": "Provide key internal links so the initial HTML contains at least 75% of rendered DOM links or differs by no more than two links.",
  "og:image, favicon, lang, hreflang 등 공유·언어 메타데이터가 실제 응답하고 현재 페이지 언어와 충돌하지 않는지도 함께 점검해 주세요.": "Also check that sharing and language metadata such as og:image, favicon, lang, and hreflang respond correctly and do not conflict with the current page language.",
  "기존 디자인과 사용자 기능을 제거하거나 비활성화하지 마세요.": "Do not remove or disable the existing design or user-facing features.",
  "초기 HTML과 렌더링 DOM의 title, meta description, H1, JSON-LD 값을 비교해 주세요.": "Compare the title, meta description, H1, and JSON-LD values between the initial HTML and rendered DOM.",
  "클라이언트 실행 후 올바른 초기 값을 오래되거나 다른 값으로 덮어쓰는 코드를 수정해 주세요.": "Fix any code that overwrites correct initial values with outdated or different values after client-side execution.",
  "WebApplication, Organization, WebSite, FAQPage 등 JSON-LD의 이름·URL·설명·FAQ 답변이 실제 화면 정보와 일치하도록 유지해 주세요.": "Keep JSON-LD names, URLs, descriptions, and FAQ answers for WebApplication, Organization, WebSite, FAQPage, and similar types consistent with the visible page information.",
  "중복되거나 충돌하는 메타데이터 선언은 하나의 정확한 값으로 정리해 주세요.": "Consolidate duplicate or conflicting metadata declarations into one accurate value.",
  "렌더링 DOM에 H1이 2개 이상이면 페이지 대표 제목 하나만 H1으로 유지하고 나머지는 일반 텍스트나 H2로 변경해 주세요.": "If the rendered DOM has two or more H1s, keep only one representative page title as H1 and change the others to regular text or H2.",
  "요금, 보안, 개인정보, 지원 범위처럼 구매 검토에 영향을 주는 정보는 화면 본문·메타데이터·JSON-LD가 서로 다른 의미로 말하지 않게 점검해 주세요.": "Check that page content, metadata, and JSON-LD do not communicate different meanings for purchase-evaluation information such as pricing, security, privacy, and support scope.",
  "페이지별로 고유하고 구체적인 title과 meta description을 초기 HTML에 제공해 주세요.": "Provide a unique and specific title and meta description for each page in the initial HTML.",
  "페이지의 주제를 설명하는 명확한 H1과 핵심 본문을 초기 HTML에 포함해 주세요.": "Include a clear H1 and core body content that explain the page topic in the initial HTML.",
  "중요한 콘텐츠 페이지, 요금제, 개인정보처리방침, 이용약관, 도움말·문의 페이지로 이동하는 일반 HTML 링크를 제공해 주세요.": "Provide standard HTML links to important content pages, pricing, privacy policy, terms, help, and contact pages.",
  "화면에 없는 내용을 검색 노출만을 위해 숨겨 넣지 말고 실제 사용자에게 보이는 정보와 일치시켜 주세요.": "Do not hide content only for search exposure; keep AI-facing information consistent with what users can actually see.",
  "중요한 내부 링크가 초기 HTML에 존재하고 렌더링 DOM과의 차이가 허용 범위입니다.": "Important internal links exist in the initial HTML, and the difference from the rendered DOM is within the allowed range.",
  "실제 화면 FAQ와 FAQPage JSON-LD의 질문·답변이 일치합니다.": "The visible FAQ questions and answers match the FAQPage JSON-LD.",
  "기존 화면 디자인과 주요 사용자 기능은 브라우저 스모크 테스트 또는 수동 확인으로 검증합니다.": "The existing screen design and major user-facing features are verified by a browser smoke test or manual check.",
  "배포 파이프라인 또는 수동 점검에 초기 HTML H1과 본문 200자 이상 확인 스모크 테스트를 추가합니다.": "Add a smoke test to the deployment pipeline or manual checklist to confirm an initial HTML H1 and at least 200 characters of body text.",
  "수정 후 ChatGPT, Perplexity, Claude 등 실제 AI 도구에 사이트 설명 질문을 해 보고 서비스 설명이 왜곡되지 않는지 확인합니다.": "After the fix, ask real AI tools such as ChatGPT, Perplexity, and Claude service-description questions and confirm that the service is not misrepresented.",
  "초기 HTML과 화면 완성 후의 페이지 제목과 설명이 같은 주제와 의미를 전달합니다.": "The page title and description in the initial HTML and after rendering communicate the same topic and meaning.",
  "대표 제목(H1)은 렌더링 DOM에 정확히 1개이며 초기 HTML과 같은 핵심 주제를 전달합니다.": "The representative H1 appears exactly once in the rendered DOM and communicates the same core topic as the initial HTML.",
  "대표 제목과 구조화 정보의 핵심 사실이 실제 화면 내용과 일치합니다.": "The core facts in the representative heading and structured data match the actual visible page content.",
  "FAQPage JSON-LD를 추가한 경우 실제 화면 FAQ와 질문·답변이 일치합니다.": "If FAQPage JSON-LD is added, its questions and answers match the visible FAQ.",
  "서로 충돌하거나 오래된 메타데이터가 남아 있지 않습니다.": "No conflicting or outdated metadata remains.",
  "재검사에서 렌더링 전후 핵심정보 불일치가 사라집니다.": "A recheck confirms that key-information inconsistencies before and after rendering are resolved.",
  "초기 HTML만 확인해도 페이지의 주제와 주요 서비스가 이해됩니다.": "The page topic and main service can be understood from the initial HTML alone.",
