...
function writeCover(
  ...
  setText(document, 7.9, COLORS.muted).text(
    "현재 보고서는 공개 URL의 실제 HTTP 응답과 초기 HTML을 기준으로 한 QUICK 진단 결과입니다. JavaScript 실행 후 DOM, 모바일·PC 비교, 업종별 기준정보와 질문 정답률은 정밀진단 단계에서 추가됩니다.",
    {
      width,
      lineGap: 3,
    },
...
function writeMethodology(
  ...
  setText(document, 9.1, COLORS.text).text(
    [
      "1. 이 보고서는 명시된 공개 URL과 검사 시점에 관찰된 실제 HTTP 응답 및 초기 HTML 증거를 기준으로 작성되었습니다.",
      ...
      "5. 현재 QUICK 진단에는 JavaScript 실행 후 DOM, 모바일·PC 비교, 업종별 기준정보와 질문 정답률이 포함되지 않습니다.",
