...
- 모든 규칙 통과 시 자동진단 최고점 99점, 확인된 실패가 있으면 최고점 98점
- 규칙 기반 종합점수·A+~F 등급·영역별 점수·점수 범위 계산
...
- 검사 결과의 주요 문제와 AI 수집 개선안 선택 기반 수정 작업지시서 생성
...
- A4 작업지시서 PDF 파일 출력
- 작업지시서 PDF에 퍼블릭 도메인 둥근모꼴+ Fixedsys TTF를 내장하여 여러 PDF 뷰어의 한글 표시·검색·복사 지원
- 현재 점수·예상 점수 범위·항목별 요구사항·완료 기준·최초 검사 증거·면책·페이지 번호 표시
- 비짓제주 실제 작업지시서 PDF A4 10페이지에서 점수 문제 5건과 점수 외 AI 수집 개선안 2건, 한글 표시·검색·복사 확인
- 검사 결과 상세 화면에서 진단 보고서 PDF 저장
- 종합점수·등급·8개 영역·AI 이해 요약·주요 문제·전체 진단·수집 페이지·검사 증거·면책·페이지 번호 표시
...
- 결과 화면과 진단 보고서에서 초기 HTML과 JavaScript 실행 후 DOM의 본문·링크·제목·JSON-LD 차이를 비교
- 렌더링 차이를 `현재 상태 → 쉬운 의미 → 무엇을 바꾸는지 → 개발자 작업 지시 → 완료 확인 기준` 구조의 AI 수집 개선안으로 제공
- 비짓제주 실제 렌더링 개선안 보고서 A4 17페이지에서 비개발자 설명·개발자 지시·완료 기준과 한글 표시·검색·복사 확인
...
- 비짓제주 작업지시서에서 기존 점수 항목 5건과 AI 수집 개선안 2건을 합친 작업 항목 7건 저장 확인

검사 신청 후 서버의 자동 백그라운드 워커가 대기 작업을 순차적으로 처리하며, 별도의 `npm run scan:once` 실행 없이 `QUEUED → RUNNING → COMPLETED` 상태 전환을 완료합니다. QUICK 검사는 초기 HTML 진단과 함께 Playwright·Chromium으로 JavaScript 실행 후 DOM을 수집합니다.

QUICK 점수는 기술 준비 37점, 신뢰 신호 13점, AI 답변 준비 콘텐츠 50점을 합산해 총 100점으로 계산합니다. 콘텐츠 8개 항목은 초기 HTML의 충분한 설명(FULL), 본문(BODY), 제목·메타·링크 단서(HINT), JavaScript 렌더링 후 확인(RENDERED), 미확인(NONE) 수준에 따라 부분 점수를 받습니다. 렌더링 후에만 확인된 정보는 부분 점수이며, 초기 HTML에도 같은 정보를 포함해야 만점을 받습니다.

렌더링 측정 실패 또는 의도적 접근 차단으로 콘텐츠 존재 여부를 확인할 수 없는 경우에는 감점하지 않고 보류 점수와 가능한 점수 범위를 표시합니다. 모든 배점 규칙을 통과해도 자동진단 최고점은 99점이며, 확인된 실패가 있으면 최고점은 98점입니다.

정밀진단의 AI 답변 성과점수는 QUICK 점수와 합산하지 않습니다. 실제 웹 검색 답변에서의 서비스 식별, 대상 사이트 인용, 사실 정확도, 완전성, 반복 일관성을 별도의 참고 지표로 측정합니다. 개선안은 비개발자용 설명, 개발자 작업 지시와 완료 확인 기준을 함께 제공하며 결과 화면과 PDF 보고서에 표시합니다. 사용자는 기존 점수 문제와 AI 수집 개선안을 선택해 작업지시서 1건으로 만들 수 있습니다. 서버는 개선안 코드를 실제 검사 증거와 다시 대조하며, 점수 외 개선안은 예상 점수 상승 범위에 포함하지 않습니다. 렌더링 실패는 전체 검사를 중단시키지 않습니다. 모바일·데스크톱 별도 비교, 에이전시 배정·배포 URL 제출·자동검수는 아직 구현하지 않았습니다. Production 공유 오브젝트 스토리지가 구성되면 현재 PostgreSQL PDF 캐시를 Cloudinary 비공개 자산 방식으로 이전할 수 있도록 저장소 구조를 분리합니다. Google 로그인과 계정 설정·회원탈퇴도 후속 단계입니다.
