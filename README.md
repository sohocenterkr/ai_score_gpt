...
- 진단 보고서에는 화면 표시 호환성이 검증된 Noto Sans KR WOFF 500 단일 글꼴을 내장하여 한글 검색·복사 지원
- 비짓제주 실제 진단 보고서 PDF A4 16페이지, 약 62KB, 전체 페이지 한글 표시 확인
...
- 실제 v1 보고서 검증에서 최초 생성 26.59초 `MISS`, 반복 다운로드 9.57ms `HIT`, PDF SHA-256 일치 확인
- 실제 v2 보고서 반복 다운로드는 약 8~13ms에 반환되고 DB 저장본과 SHA-256 일치 확인
- 비짓제주 발급 작업지시서 1건과 작업 항목 5건 저장 확인

자동 백그라운드 실행, Playwright 렌더링 검사, 에이전시 배정·배포 URL 제출·자동검수는 아직 구현하지 않았습니다. 진단 보고서 PDF 최초 생성 성능은 후속 최적화 대상이며, Production 공유 오브젝트 스토리지가 구성되면 현재 PostgreSQL PDF 캐시를 Cloudinary 비공개 자산 방식으로 이전할 수 있도록 저장소 구조를 분리합니다. Google 로그인과 계정 설정·회원탈퇴도 후속 단계입니다.
