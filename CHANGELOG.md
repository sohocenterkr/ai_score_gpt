...
- 쇼핑몰 직접 결제 여부를 사이트 등록·수정·재진단에서 명시적으로 선택할 수 있도록 추가
- 예약·상담 미선택이 상품·결제 증거보다 먼저 정보형으로 확정되던 전환 유형 판정 오류 수정
- Product/Offer, 결제 모듈, 장바구니·재고, 상품·가격 증거를 이용한 전자상거래 자동 판정과 충돌 근거 기록 추가
- 쇼핑몰 작업지시서에 SaaS 전용 가입·요금제·WebApplication 문구가 섞이지 않도록 업종별 문구 보정
- 실제 실패 항목이 없는 PDF Epic과 불필요한 SSR/SSG 필수 게이트 출력 방지
- 템플릿 변수가 포함된 잘못된 URL을 정상 내부 링크 집계에서 제외
- 정밀진단 기본 팩트와 질문을 브랜드·상품·매장·예약 서비스·SaaS에 공통 적용 가능한 표현으로 개선
...
- QUICK 점수 체계를 `2026.08-site-archetype-v8` 기준으로 문서화
- 총 100점을 기술 준비 37점, 신뢰 신호 13점, AI 답변 준비 콘텐츠 50점으로 구분
- 콘텐츠 8개 항목에 초기 HTML·본문·단서·JavaScript 렌더링 근거 수준별 부분 점수 기준 반영
- 렌더링 측정 실패 또는 의도적 접근 차단으로 확인할 수 없는 콘텐츠는 감점하지 않고 보류 점수와 점수 범위로 표시
- 자동진단 최고점을 모든 규칙 통과 시 99점, 확인된 실패가 있을 때 98점으로 문서화
- 실제 적용 중인 치명적 점수 상한을 접속 실패 10점, 전체 noindex 30점, HTML 콘텐츠 추출 실패 50점으로 정리
- 정밀진단 AI 답변 성과 방법론 `2026.06-ai-answer-v4`의 서비스 식별 기반 브랜드 기여·인용·정확도·완전성·일관성 계산과 상한 기준 문서화
