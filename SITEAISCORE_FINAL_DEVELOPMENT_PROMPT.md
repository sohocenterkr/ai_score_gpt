...
현재 QUICK 규칙 버전은 `2026.07-summary-groups-v7.7`이며, 총 배점은 100점입니다. QUICK는 공개 URL에서 확인한 기술·구조·콘텐츠 증거를 규칙으로 계산하는 진단이며, 정밀진단의 실제 AI 답변 성과점수와 합산하지 않습니다.

| 요약 그룹 | 배점 | 구성 |
|---|---:|---|
| 기술 준비 | 37 | 접근성, robots, sitemap, llms.txt, 초기 HTML, 제목·메타·언어·링크·색인 상태 |
| 신뢰 신호 | 13 | JSON-LD 존재·유형·sameAs·contactPoint·SearchAction·엔터티 신뢰 |
| AI 답변 준비 콘텐츠 | 50 | 서비스 정의, 대상·활용 사례, 이용 절차·결과물, 요금, 지원, 데이터 처리, 차별점, 거래·문의 정책 |

세부 영역은 접근 및 수집 정책 9점, 콘텐츠 읽기 용이성 9점, 정보 구조와 의미 전달 7점, 핵심정보 인식 정확도 13점, 콘텐츠 이해 및 답변 가능성 5점, AI 에이전트 사용 가능성 5점, 최신성 및 측정 환경 2점, AI 답변 준비 콘텐츠 50점입니다.

## 18.1 QUICK 콘텐츠 증거와 부분 점수

AI 답변 준비 콘텐츠 8개 항목은 실제 공개 정보의 근거 수준에 따라 부분 점수를 계산합니다.

| 근거 수준 | 점수 처리 |
|---|---|
| FULL | 초기 HTML에서 충분한 설명과 구조를 확인하여 해당 항목 만점 |
| BODY | 본문 문장에서 관련 정보를 확인하여 부분 점수 |
| HINT | 제목·메타데이터·링크의 짧은 단서만 확인하여 낮은 부분 점수 |
| RENDERED | JavaScript 실행 후에만 정보를 확인하여 부분 점수 |
| NONE | 관련 정보를 확인하지 못해 0점 |
| UNAVAILABLE | 렌더링 측정 실패 등으로 존재 여부를 확인하지 못해 감점하지 않고 보류 |

RENDERED는 화면에는 정보가 보이지만 초기 HTML에는 없다는 뜻입니다. GPTBot 등 다수 AI 크롤러가 JavaScript를 실행하지 않을 수 있으므로, 같은 정보를 서버 렌더링 또는 정적 초기 HTML에 포함해야 FULL 만점을 받을 수 있습니다.

의도적인 접근 차단으로 진단 서버가 사이트를 확인할 수 없는 경우에도 관련 점수는 확정 감점하지 않고 보류합니다. 결과에는 확정 점수, 보류 점수, 가능한 최고 점수 범위를 함께 표시합니다.

## 18.2 점수·등급·자동진단 상한

모든 배점 규칙을 통과해도 자동진단 최고점은 99점입니다. 확인된 실패가 하나라도 있으면 자동진단 최고점은 98점입니다. 공개 URL의 자동 측정만으로 100점 완성을 보증하지 않기 위한 기준입니다.
...
다음 치명적 조건은 계산된 점수에 상한을 적용합니다.

- 사이트 전체 접속 실패: 최대 10점
- 전체 noindex: 최대 30점
- 메인 HTML 콘텐츠를 추출할 수 없음: 최대 50점

OAI-SearchBot, ChatGPT-User, GPTBot 정책과 실제 요청 결과는 수집 증거로 표시합니다. 현재 OAI-SearchBot과 GPTBot의 개별 항목은 0점 관찰 항목이며, 이 항목 하나의 실패만으로 점수 상한을 적용하지 않습니다.

## 18.3 정밀진단 AI 답변 성과점수

정밀진단은 QUICK와 별도로 실제 웹 검색 기반 AI 답변의 성과를 측정합니다. 현재 방법론 버전은 `2026.06-ai-answer-v4`입니다.

- 서비스 식별을 동반한 브랜드 기여율 15%
- 대상 사이트 인용률 30%
- 사실 정확도 30%
- 답변 완전성 15%
- 반복 일관성 10%

대상 서비스를 식별하지 못한 단순 브랜드 문자열 언급은 브랜드 기여로 인정하지 않습니다. 사실 정확도와 완전성이 측정되지 않으면 종합 성과점수를 만들지 않습니다. 이 점수는 QUICK 점수나 등급을 임의로 변경하지 않고, 실제 AI 답변 환경의 별도 참고 지표로 표시합니다.

## 18.4 규칙 엔진과 재비교

한 검사에서는 시작 시점의 규칙 버전을 고정합니다. 새 규칙 버전이 배포된 뒤에는 이전 결과를 현재 규칙과 구분해 표시하고, 새 진단을 실행해야 현재 기준으로 다시 계산합니다.

규칙별로 코드, 영역, 요약 그룹, 배점, 판정, 증거, 권장사항, 규칙 버전을 저장합니다. 감점 또는 부분 점수에는 대상 URL과 확인값·근거를 남기며, 근거가 부족해 확인할 수 없는 콘텐츠는 감점 대신 보류로 처리합니다.
